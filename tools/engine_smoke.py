...
def main():
    ...
    # Incremental autosave must not treat unvisited bodies as deleted when
    # the wall-clock budget aborts the roster walk mid-pass.
    _assert_incomplete_autosave_does_not_delete_unvisited()

    print("engine_smoke_ok")
    return 0


def _assert_incomplete_autosave_does_not_delete_unvisited():
    """Regression: wall-budget abort + dirty row must not DELETE the rest."""
    import asyncio
    import sqlite3
    import time

    from engine import persistence as persist
    from engine.world import Character, Room
    ...
    class _PersistGame:
        def __init__(self):
            ...
            self._persist_warm = False
            self._persist_force_full = False
            self._persist_char_snapshot_hashes = None
            self._persist_floor_room_hashes = None
            self._persist_dirty_characters = set()
            self._persist_floor_item_rooms = set()
            self._persist_dirty_floor_rooms = set()

        def find_character(self, key):
            for obj in self.start_room.contents:
                if getattr(obj, "key", None) == key:
                    return obj
            return None

    game = _PersistGame()
    room = Room("limbo", "Nowhere.")
    game.rooms[room.key] = room
    game.start_room = room
    chars = []
    for i in range(12):
        ch = Character(f"P{i:02d}")
        ch.account = f"acct{i:02d}"
        ch.is_npc = False
        ch.move_to(room)
        chars.append(ch)
    persist.seed_snapshot_hashes_after_load(game)
    game._persist_dirty_characters = {chars[0].key}

    conn = sqlite3.connect(":memory:")
    conn.execute(
        "CREATE TABLE characters "
        "(name TEXT PRIMARY KEY, description TEXT, room_key TEXT, stats TEXT)"
    )
    conn.execute(
        "CREATE TABLE items "
        "(key TEXT, description TEXT, holder_type TEXT, "
        "holder_key TEXT, container TEXT)"
    )
    for ch in chars:
        conn.execute(
            "INSERT INTO characters VALUES (?, ?, ?, ?)",
            (ch.key, ch.description, room.key, "{}"),
        )
    conn.commit()

    checks = {"n": 0}
    orig = persist._persist_save_over_wall_budget

    def _trip_after(_start, _budget):
        checks["n"] += 1
        return checks["n"] > 3

    persist._persist_save_over_wall_budget = _trip_after
    try:
        char_rows, item_rows, meta = asyncio.run(
            persist._collect_world_save_snapshot_async(
                game,
                yield_every=0,
        ...
        persist._persist_save_over_wall_budget = orig

    assert meta.get("wall_budget_hit") is True
    assert meta.get("scan_incomplete") is True
    assert meta.get("changed_chars") == ["P00"]
    assert meta.get("removed_chars") == []
    persist._apply_world_save_snapshot(conn, char_rows, item_rows, meta)
    names = {row[0] for row in conn.execute("SELECT name FROM characters")}
    assert names == {ch.key for ch in chars}, names
