...
def get_mine_room(game, mouth_key, room_id):
    ...
    # Exits from carved faces.
    for direction, face in (node.get("exits") or {}).items():
        if not face.get("carved"):
            continue
        dest_id = face.get("dest_room_id")
        if dest_id:
            dest_key = mine_room_key(mouth_key, dest_id)
            dest_room = get_mine_room(game, mouth_key, dest_id)
            if dest_room:
...
def wilderness_mouth_room(game, mouth_key):
    ...
    if parsed is None:
        return None
    map_id, mx, my, ux, uy = parsed
    from engine.systems.overland import get_virtual_room
    return get_virtual_room(game, (mx, my), (ux, uy))
