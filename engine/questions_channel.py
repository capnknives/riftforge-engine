...
def render_questions_line(character, face: str, message: str) -> str:
    ...
    Same layered ``((MARK))`` chrome as OOC (crimson parens, gold letters)
    so the help net does not blend into room prose. Player text is never
    tag-parsed. ``config channel questions <role>`` paints the whole line.
    """
    ...
    if display_prefs.wants_plain_comms(character):
        return display_prefs.paint_plaincomms_channel_lead(
            character, plain, "Questions.", body_role="ooc",
        )
    prefix = "((QUESTIONS))"
    ...
    return display_prefs.paint_double_bracket_channel_line(
        character, "QUESTIONS", plain[len(prefix):], channel="questions",
    )
    ...
    *prefix* prepends to the message body (e.g. ``[Query #3]`` cross-posts).
    ...
    for session in list(getattr(game, "sessions", None) or []):
        ...
        if speaker is not None:
            ...
            face = public_face
        line = render_questions_line(other, face, body)
        session.send(line)
        session.send("")
        gmcp.push_comm(session, "questions", body, face)
        delivered = True
    if not delivered and speaker_session is not None and speaker is not None:
        face = speaker_face_for_character(speaker, game, viewer=speaker)
        line = render_questions_line(speaker, face, body)
